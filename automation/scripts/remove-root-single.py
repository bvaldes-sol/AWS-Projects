...
─────
  ...
  python remove_root_credentials.py \
      --account-id 123456789012                            # single account, dry-run
  python remove_root_credentials.py --execute \
      --account-id 123456789012                            # single account, execute
...
class RootCredentialManager:
    ...
    audits root credentials, then removes them (unless dry_run=True).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.account_id   = account_id          # None → run against whole org
    ...
    def _resolve_accounts(self) -> list[dict]:
        ...
        If --account-id was supplied: validate the format and return just that
        one account (bypassing Organizations entirely — no ListAccounts call).
        Otherwise: paginate the full org, respecting --exclude.
        """
        if self.account_id:
            # Basic format guard — AWS account IDs are exactly 12 digits
            ...
            # Use the account ID as the name placeholder; we don't call orgs here
...
examples:
  ...
  python remove_root_credentials.py --account-id 123456789012
  python remove_root_credentials.py --execute --account-id 123456789012
        """,
    )
    parser.add_argument(
        "--execute",
    ...
    parser.add_argument(
        ...
        metavar="ACCOUNT_ID",
        help=(
            "Target a single member account instead of the whole organization. "
    ...
    parser.add_argument(
        ...
        default="",
        metavar="ID1,ID2",
        help="Comma-separated account IDs to skip (ignored when --account-id is set).",
    )
    parser.add_argument(
        "--profile",
        default=None,
