"""
manage_root_credentials.py
──────────────────────────────────────────────────────────────────────────────
Manages root user credentials across member accounts in an AWS Organization.

Two operations are supported:

  REMOVE (default)
    Deletes root credentials (login profile, access keys, signing certificates,
    and MFA devices) from every member account — or a single specified account.

  ENABLE RECOVERY  (--enable-recovery)
    Enables password recovery for the root user on a specific account so the
    person with access to the root email inbox can reset the password themselves.
    Requires --account-id — enabling recovery org-wide is intentionally blocked.

Prerequisites
─────────────
• Run from the management account or a delegated IAM administrator account.
• The calling principal must have:
    - organizations:ListAccounts   (remove org-wide only)
    - sts:AssumeRoot
...
Usage
─────
  # REMOVE — dry-run (safe default, no changes made)
  python manage_root_credentials.py

  # REMOVE — apply to all member accounts
  python manage_root_credentials.py --execute

  # REMOVE — apply, skipping specific accounts
  python manage_root_credentials.py --execute --exclude 111122223333,444455556666

  # REMOVE — apply to a single account
  python manage_root_credentials.py --execute --account-id 123456789012

  # ENABLE RECOVERY — dry-run for a single account
  python manage_root_credentials.py --enable-recovery --account-id 123456789012

  # ENABLE RECOVERY — apply to a single account
  python manage_root_credentials.py --enable-recovery --execute --account-id 123456789012

  python manage_root_credentials.py --help
...
mapped to typed exceptions:
  ...
  CredentialRemovalError — one operation step failed; log it and keep going
...
RECOVERY_TASK_POLICY_ARN = "arn:aws:iam::aws:policy/root-task/IAMCreateRootUserPassword"
...
class AccountCredentialSummary:
    ...
    skipped:             bool = False
    skip_reason:         str  = ""
    recovery_enabled:    bool = False   # set to True when enable-recovery succeeds
...
class SessionFactory:
    ...
    sts:AssumeRoot with the appropriate task policy.

    Two task policies are supported:
      TASK_POLICY_ARN          — scoped for deleting root credentials
      RECOVERY_TASK_POLICY_ARN — scoped for creating a login profile (enable recovery)
    ...
    def get_root_iam_client(self, account_id: str, task_policy_arn: str = TASK_POLICY_ARN):
        """
        Returns a boto3 IAM client with temporary root credentials scoped to
        account_id using the given task_policy_arn.
...
class RootRecoveryEnabler:
    """
    Enables password recovery for the root user on a single member account by
    calling iam:CreateLoginProfile() via a scoped AssumeRoot session.

    What this does
    ──────────────
    CreateLoginProfile() in an AssumeRoot session does NOT set a password —
    AWS intentionally does not allow that programmatically.  Instead it marks
    the account so the person with access to the root email inbox can trigger
    a password reset themselves through the normal AWS recovery flow.

    What this does NOT do
    ─────────────────────
    • It does not set or expose a password.
    • It does not grant anyone immediate root access.
    • It does not create access keys or any other credential type.

    EntityAlreadyExists
    ───────────────────
    If a login profile already exists (recovery was already enabled or root
    credentials were never fully removed), AWS returns EntityAlreadyExists.
    This is treated as a benign no-op — the account is already in the desired
    state and the summary reflects that.
    """
    ...
    def enable_recovery(self, summary: AccountCredentialSummary) -> None:
        """
        Calls iam:CreateLoginProfile to enable password recovery on the account.
        Result (success or failure) is recorded in summary.
        """
        account_id = summary.account_id

        try:
            resp = AWSCaller.invoke(
                self._iam.create_login_profile,
                _context      = "iam:CreateLoginProfile (enable recovery)",
                _account_id   = account_id,
                # EntityAlreadyExists → recovery already enabled, nothing to do
                _benign_codes = frozenset({"EntityAlreadyExists"}),
            )

            if resp is None:
                # Benign — login profile already existed
                self._log.info(
                    "  Password recovery was already enabled — no change made."
                )
                summary.actions_taken.append(
                    "Password recovery already enabled (EntityAlreadyExists — no change)"
                )
            else:
                summary.recovery_enabled = True
                summary.actions_taken.append(
                    "Enabled password recovery — root email holder can now reset the password"
                )

        except CredentialRemovalError as exc:
            err = f"Failed to enable password recovery: {exc}"
            summary.errors.append(err)
            self._log.error(f"  ✗ {err}")
...
class RootCredentialManager:
    """
    Iterates every active member account (or a single specified account),
    audits root credentials, then removes them or enables recovery depending
    on the operation mode selected.

    Operation modes
    ───────────────
    enable_recovery=False (default)  — remove all root credentials
    enable_recovery=True             — enable password recovery on the target
                                       account (requires account_id to be set)
    """
    ...
    def run(self) -> list[AccountCredentialSummary]:
        operation = "ENABLE PASSWORD RECOVERY" if self.enable_recovery else "REMOVE ROOT CREDENTIALS"
        mode      = "DRY RUN — no changes will be made" if self.dry_run else "EXECUTE"

        self._log.info("=" * 70)
        self._log.info(f"  Operation : {operation}")
        self._log.info(f"  Mode      : {mode}")
        self._log.info("=" * 70)

        accounts = self._resolve_accounts()
        self._log.info(f"Found {len(accounts)} account(s) to process.\n")

        for acct in accounts:
            if self.enable_recovery:
                summary = self._process_account_recovery(acct["Id"], acct["Name"])
            else:
                summary = self._process_account_remove(acct["Id"], acct["Name"])
            self._results.append(summary)

        self._print_report()
        return self._results

    def _resolve_accounts(self) -> list[dict]:
        """Return the list of accounts to operate on.

        Enable-recovery mode: --account-id is required — enabling recovery
        org-wide is intentionally blocked as a safety measure.

        Remove mode: if --account-id is supplied, target just that one account
        (no Organizations call needed); otherwise paginate the full org.
        """
        if self.enable_recovery:
            if not self.account_id:
                raise FatalConfigError(
                    "--account-id is required when using --enable-recovery. "
                    "Enabling password recovery org-wide is not permitted — "
                    "specify a single target account."
                )
            # Validation for the account ID format is done below
            if not self.account_id.isdigit() or len(self.account_id) != 12:
                raise FatalConfigError(
                    f"--account-id '{self.account_id}' is not a valid 12-digit "
                    "AWS account ID."
                )
            self._log.info(f"  Scope: single account {self.account_id} (enable recovery)")
            return [{"Id": self.account_id, "Name": self.account_id}]
        ...
        if self.account_id:
            ...
            self._log.info(f"  Scope: single account {self.account_id}")
            return [{"Id": self.account_id, "Name": self.account_id}]

        self._log.info("  Scope: all active member accounts in the organization")
        return self._lister.list_accounts(self.excluded_ids)

    # ── private helpers ───────────────────────────────────────────────────────

    def _get_scoped_iam_client(
        self,
        account_id:    str,
        account_name:  str,
        task_policy:   str,
        ...
        """
        Shared helper — obtains a scoped root IAM client for an account using
        the given task policy ARN.  Returns the client on success, or returns
        None and appends a skipped summary if access is denied.
        Raises FatalConfigError for script-wide problems.
        """
        try:
            return self._factory.get_root_iam_client(account_id, task_policy)
    ...
    def _process_account_remove(self, account_id: str, account_name: str) -> AccountCredentialSummary:
        """Full remove flow: audit credentials then delete them."""
        self._log.info(f"── {account_name} ({account_id})")

        iam_client = self._get_scoped_iam_client(
            account_id, account_name, TASK_POLICY_ARN
        )
        if iam_client is None:
            return AccountCredentialSummary(
                ...
                account_name = account_name,
                skipped      = True,
                skip_reason  = "AssumeRoot denied (see WARNING above)",
            )

        # Audit current credential state
    ...
    def _process_account_recovery(self, account_id: str, account_name: str) -> AccountCredentialSummary:
        """Enable-recovery flow: call CreateLoginProfile to allow root email reset."""
        self._log.info(f"── {account_name} ({account_id})")

        iam_client = self._get_scoped_iam_client(
            account_id, account_name, RECOVERY_TASK_POLICY_ARN
        )
        if iam_client is None:
            return AccountCredentialSummary(
                account_id   = account_id,
        ...
        if self.dry_run:
            self._log.info(
                "  [DRY RUN] Would call iam:CreateLoginProfile to enable "
                "password recovery for the root email holder.\n"
            )
            return summary

        enabler = RootRecoveryEnabler(iam_client, self._log)
        enabler.enable_recovery(summary)
    ...
    def _print_report(self) -> None:
        ...
        errored = [s for s in self._results if not s.skipped and s.errors]

        self._log.info(f"  Total accounts  : {len(self._results)}")
        self._log.info(f"  Skipped         : {len(skipped)}")

        if self.enable_recovery:
            recovered = [s for s in self._results if not s.skipped and s.recovery_enabled]
            already   = [
                s for s in self._results
                if not s.skipped and not s.recovery_enabled and not s.errors
            ]
            self._log.info(f"  Recovery enabled: {len(recovered)}")
            self._log.info(f"  Already enabled : {len(already)}")
        else:
            no_creds  = [
                s for s in self._results
                if not s.skipped and not s.has_any_credentials and not s.errors
            ]
            processed = [
                s for s in self._results
                if not s.skipped and (s.has_any_credentials or s.errors)
            ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Manage root user credentials across AWS Organization member accounts.\n\n"
            "Default operation: REMOVE all root credentials (org-wide or single account).\n"
            "With --enable-recovery: enable password recovery on a single account."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
examples:
  # REMOVE — dry-run against all member accounts (safe default)
  python manage_root_credentials.py
  ...
  # REMOVE — dry-run against a single account
  python manage_root_credentials.py --account-id 123456789012

  # REMOVE — apply to a single account
  python manage_root_credentials.py --execute --account-id 123456789012

  # ENABLE RECOVERY — dry-run (shows what would happen)
  python manage_root_credentials.py --enable-recovery --account-id 123456789012
  ...
  python manage_root_credentials.py --enable-recovery --execute --account-id 123456789012
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable password recovery for the root user on the target account "
            "instead of removing credentials. Requires --account-id. "
            "This allows the root email holder to reset the password themselves — "
            "it does not set or expose any password."
        ),
    )
    parser.add_argument(
        "--account-id",
        ...
        metavar="ACCOUNT_ID",
        help=(
            "Target a single member account. Required when using --enable-recovery. "
            "For remove operations, bypasses the org-wide account list."
        ),
    )
    parser.add_argument(
        "--exclude",
        default="",
        metavar="ID1,ID2",
        help="Comma-separated account IDs to skip (remove mode, org-wide only).",
    )
    parser.add_argument(
        "--profile",
        default=None,
